...
- validator 已能发现 stdlib function/macro 对应 module 的缺失声明；本轮据此补齐
  `surfaceflinger_module` 对 `android.frames.jank_type` 的依赖。
...
- 尚未完成：syntaqlite formatting/diagnostics 接入 AI SQL 展示和复制、
  `stdlib_docs.json`/pfsql lineage、Connection/Database 生命周期映射、
  upstream Android 性能功能迁移、upstream AI skills 转译。
...
| M2 | `stdlib_docs.json` + pfsql lineage 接入 SQL 知识基座 | Next |
| M3 | Trace Processor Connection/Database split 与 trace lease/pool 映射 | Todo |
| M4 | Heap/lock/jank/startup 等 upstream 功能产品化 | Todo |
| M5 | upstream AI skills 查询和判断标准转译 | Todo |
...
3. 然后升级 Android 性能 Skill，把 upstream 新 stdlib 能力转化成
   SmartPerfetto 的 evidence layer。
4. 最后处理多 Trace/Connection 架构和 UI 产品化。这两块影响面更大，需要在
   前三步稳定后再改。
