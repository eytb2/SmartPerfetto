...
| Load-test report includes p50/p95, error rate, worker RSS, queue length, LLM cost, trace metadata scale, and daily LLM call projection | Open | `enterpriseAcceptanceLoadTest.ts` emits these metrics from HTTP samples, workspace trace-list responses, and `/api/admin/runtime`; acceptance now requires at least 1,000 visible trace metadata entries, runtime RSS, queue length, measurable LLM cost delta, an LLM call-count increase from the pre-run runtime baseline, and at least 200 estimated LLM calls per day when projected from the measured load-test window. Requires `load-test-report.md` from a real 50-user load run. Current RSS benchmark is separately blocked by missing large traces. |
