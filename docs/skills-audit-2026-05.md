...
| ID | task | 状态 |
|----|------|------|
| Other.1 | M0.5b 前端 `expandableData` 排序契约 | ✓ 完成：`SqlResultTable` 以 row + expandable payload 为同一个显示单元排序，并用原始行号维护展开状态；已补单测；perfetto draft PR #8 |
...
4. **Tier-3 剩余**：M1 逐 skill 合规 sweep / M2-M5 composite 设计 / fixture 采集 / 架构 specific jank
